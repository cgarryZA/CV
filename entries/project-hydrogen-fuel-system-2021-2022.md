---
id: project-hydrogen-fuel-system-2021-2022
title: Hydrogen Fuel System for Gas Turbines (Y2 Project)
date: 2025-11-03
type: project
section: Research
institution: Durham University
location: Durham, United Kingdom
period: Oct 2021 – Apr 2022
links.linkedin: https://www.linkedin.com/in/christian-tt-garry/
links.github:
links.education: ./meng-electronic-2024.md
cover: assets\durham_university_logo.jpeg
tags: [Team Project, Hydrogen, Cryogenics, Sensors, Aerospace]
---

### Hydrogen Fuel System for Gas Turbines — Year 2 Project  
*Team of 5; cryogenic storage, sensing, and safety systems for passenger jets*

**My focus**  
- Designed **leak detection** architecture and integrated **tank sensors** for temperature, pressure, and level monitoring.  
- Modelled **boil‑off** behaviour and contributed to **pressure compensation** systems for stable turbine feed.  

**Outcome**  
Concept validated against safety/operational constraints; delivered engineering report and presentations. Recieved 77% for the final project.

---

### Short CV Snippet (LaTeX)

```latex
\begin{twocolentry}
    {Durham, United Kingdom}
    \textbf{Hydrogen Fuel System (Y2 Project)}
\end{twocolentry}

\begin{twocolentry}
    {Oct 2021 – Apr 2022}
    \textbf{Durham University}
\end{twocolentry}

\vspace{0.10 cm}
\begin{onecolentry}
    \begin{highlights}
        \item Designed cryogenic fuel system with leak detection and pressure monitoring for passenger jets.
    \end{highlights}
\end{onecolentry}
```
