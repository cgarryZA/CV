...
# === Tailored-CV selection (jobs domain) ===
# When CV_SELECTION points at a tailored iter-NN/selection.json, restrict the
# CV to the entries it names, in the order it gives, and inject its profile
# summary. Absent → the canonical full CV builds exactly as before.

def _load_selection():
    path = os.getenv("CV_SELECTION")
    if not path:
        return None
    p = Path(path)
    if not p.is_absolute():
        p = ROOT / p
    if not p.exists():
        print(f"[WARN] CV_SELECTION={path} not found; building full CV")
        return None
    try:
        sel = json.loads(p.read_text(encoding="utf-8"))
        print(f"[OK] Tailored build from {path}")
        return sel
    except Exception as exc:  # noqa: BLE001
        print(f"[WARN] could not parse {path}: {exc}; building full CV")
        return None
...
def _entry_keys(meta: dict, md: Path) -> set:
    """The identifiers a selection might reference this entry by."""
    keys = {meta.get("id"), md.stem, md.name, f"entries/{md.name}"}
    keys.discard(None)
    return keys
...
def main():
    ...
    ordered = selection.get("ordered_entry_ids") if selection else None
    order_index = {str(k): i for i, k in enumerate(ordered)} if ordered else None
    ...
    for md in sorted(ENTRIES_DIR.glob("*.md")):
        ...
        if order_index is not None:
            hits = [order_index[k] for k in _entry_keys(meta, md) if k in order_index]
            if not hits:
                continue  # not selected for this tailored CV
            order = min(hits)
    ...
    # Tailored profile summary — only when a selection supplies one.
    if selection and selection.get("summary"):
        out.append("\n\\section*{Profile}\n")
        out.append(_latex_escape(selection["summary"]))
    ...
    for key, title in SECTION_ORDER:
        ...
        out.append(f"\n\\section*{{{title}}}\n")
        if order_index is not None:
            items.sort(key=lambda x: x["order"] if x["order"] is not None else 10**9)
        else:
